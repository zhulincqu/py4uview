...
class UvIm(DataArray):
    ...
    def _load_file(self):
        ...
        def read_varian(header, current_position):
            ...
            val = struct.unpack('<f', header[positio n +len(temp_1 ) +len(temp_2)
                                + 3:positio n +len(temp_1 ) +len(temp_2 ) +7])[0]
        ...
        with open(self.filename, 'rb') as f:
            ...
            for b in b_iter:
                ...
                # Average Images = 255 : Sliding Averaging
                    ...
                    self.metadata['Camera Exposure'] = [struct.unpack('<f', img_header[positio n +1:positio n +5])[0], 's']
                    self.metadata['Average Images'] = img_header[positio n +5]
                    logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
                        b, 'Camera Exposure:',
                ...
                # Micrometers(x,y)
                    ...
                    self.metadata['Mitutoyo X'] = \
                        [struct.unpack('<f', img_header[positio n +1:positio n +5])[0], 'mm']
                    self.metadata['Mitutoyo Y'] = \
                        [struct.unpack('<f', img_header[positio n +5:positio n +9])[0], 'mm']
                    logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
                ...
                # FOV rotation from LEEM presets
                    ...
                    self.metadata['Rotation'] = \
                        [struct.unpack('<f', img_header[positio n +1:positio n +5])[0], 'degree']
                    logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
                ...
                # Theta and Phi
                    ...
                    self.metadata['Theta'] = \
                        [struct.unpack('<f', img_header[positio n +1:positio n +5])[0], 'degree']
                    self.metadata['Phi'] = \
                        [struct.unpack('<f', img_header[positio n +5:positio n +9])[0], 'degree']
                    logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
