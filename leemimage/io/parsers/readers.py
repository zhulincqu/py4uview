...
from leemimage._typing import (
    FilePathOrBuffer,
)


def read_uv_dat(filepath_or_buffer: FilePathOrBuffer):
    return FileReader(filepath_or_buffer)
...
class FileReader():
    ...
    def _read_field(self):
        ...
        temp = self._header[self._position + 1:].split(b'\x00')[0]
        name = temp[:-1].decode('cp1252')
        if sys.version_info[0] > 2:
            unit_tag = int(chr(temp[-1]))
        else:
            unit_tag = int(temp[-1])
        val = struct.unpack('<f', self._header[self._position + len(temp)
                                               + 2:self._position + len(temp) + 6])[0]
        offset = len(temp) + 5  # length of entire field
        return name, UNIT_DICT[unit_tag], val, offset

    def _read_imgheader(self):

        b_iter = iter(self._header)
        ...
        for b in b_iter:
            ...
            # Average Images = 255 : Sliding Averaging
                ...
                self.metadata['Camera Exposure'] = [
                    struct.unpack('<f', self._header[self._position + 1:self._position + 5])[0], 's']
                self.metadata['Average Images'] = self._header[self._position + 5]
                logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
                    b, 'Camera Exposure:',
            ...
            # MCP screen
                ...
                self.metadata['MCPscreen'] = [
                    struct.unpack('<f', self._header[self._position + 1:self._position + 5])[0], 'V']
                logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
            ...
            # Micrometers(x,y)
                ...
                self.metadata['Mitutoyo X'] = \
                    [struct.unpack('<f', self._header[self._position + 1:self._position + 5])[0], 'mm']
                self.metadata['Mitutoyo Y'] = \
                    [struct.unpack('<f', self._header[self._position + 5:self._position + 9])[0], 'mm']
                logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
                ...
                self.metadata['MirrorState'] = self._header[self._position + 1]
            ...
            # FOV
                ...
                self.metadata['FOV cal. factor'] = \
                    float(struct.unpack('<f',
                                        self._header[self._position + len(temp) + 2: self._position + len(temp) + 6])[
                              0])
                ...
                logging.info(
                    '\t{:>3}\t{:<18}\t{}'.format('', 'FOV cal. factor:', self.metadata['FOV cal. factor']))
                offset = len(temp) + 5

            # FOV rotation from LEEM presets
            elif b == 113:
                self.metadata['Rotation'] = \
                    [struct.unpack('<f', self._header[self._position + 1:self._position + 5])[0], 'degree']
                logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
                    b, 'Rotation:', self.metadata['Rotation'][0], self.metadata['Rotation'][1]))
                offset = 4
            # Spin up or down
            elif b == 240:
                self.metadata['Spin up_down'] = self._header[self._position + 1]
            ...
            # Theta and Phi
                ...
                self.metadata['Theta'] = \
                    [struct.unpack('<f', self._header[self._position + 1:self._position + 5])[0], 'degree']
                self.metadata['Phi'] = \
                    [struct.unpack('<f', self._header[self._position + 5:self._position + 9])[0], 'degree']
                logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
    ...
    def _read(self, filepath_or_buffer: FilePathOrBuffer):
        ...
        with open(filepath_or_buffer, 'rb') as f:
            ...
            self.metadata['timestamp'] = _convert_ad_timestamp(struct.unpack('<Q', f.read(8))[0])
            ...
            versleemdata = struct.unpack('<h', f.read(2))[0]
            ...
            if versleemdata == 2:
                ...
                f.seek(388, 1)
                header = f.read(versleemdata)

            #### DEBUG ####
            logging.debug('type(img_header) = {}'.format(type(header)))
            ###############
            self._header = header
