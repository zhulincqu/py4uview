...
class FileReader():
    ...
    def _read_field(self):
        ...
        if sys.version_info[0] > 2:
            ...
            unit_tag = int(temp[-1])
        val = struct.unpack('<f', self._leemheader[self._position + len(temp)
        ...
        for b in b_iter:
            ...
            # Average Images = 255 : Sliding Averaging
                ...
                self.metadata['Camera Exposure'] = [
                    struct.unpack('<f', self._leemheader[self._position + 1:self._position + 5])[0], 's']
            ...
            # MCP screen
                ...
                self.metadata['MCPscreen'] = [
                    struct.unpack('<f', self._leemheader[self._position + 1:self._position + 5])[0], 'V']
                logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
            ...
            # Micrometers(x,y)
                ...
                self.metadata['Mitutoyo X'] = \
                    [struct.unpack('<f', self._leemheader[self._position + 1:self._position + 5])[0], 'mm']
                self.metadata['Mitutoyo Y'] = \
                    [struct.unpack('<f', self._leemheader[self._position + 5:self._position + 9])[0], 'mm']
                logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
            ...
            # FOV
                ...
                self.metadata['FOV cal. factor'] = \
                    float(struct.unpack('<f',
                                        self._leemheader[self._position + len(temp) + 2: self._position + len(temp) + 6])[
                              0])
            ...
            # FOV rotation from LEEM presets
                ...
                self.metadata['Rotation'] = \
                    [struct.unpack('<f', self._leemheader[self._position + 1:self._position + 5])[0], 'degree']
                logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
            ...
            # Theta and Phi
                ...
                self.metadata['Theta'] = \
                    [struct.unpack('<f', self._leemheader[self._position + 1:self._position + 5])[0], 'degree']
                self.metadata['Phi'] = \
                    [struct.unpack('<f', self._leemheader[self._position + 5:self._position + 9])[0], 'degree']
                logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(
    ...
    def _read_varian(self):
        ...
        val = struct.unpack('<f', self._leemheader[self._position + len(temp_1) + len(temp_2)
                                               + 3: self._position + len(temp_1) + len(temp_2) + 7])[0]
        offset = len(temp_1) + len(temp_2) + 6  # length of entire field
        logging.info('\t{:>3}\t{:<18}\t{:g} {}'.format(self._leemheader[self._position],
                                                       str_1 + ':', val, str_2))
